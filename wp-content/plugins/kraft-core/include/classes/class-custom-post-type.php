...
if( ! class_exists( 'CPT' ) ) {	 
	...
	class CPT {
		 ...
		 */
		function register_post_taxonomy($taxonomy_names, $options = array()) {
		 ...
		 *   
		 */
		function submenu_page_settings( $options = array() ) {

			$this->submenu_page_options = $options;		
		}
		...
		function register_submenu_page() {
			...
			$parent_slug	   = $this->submenu_page_options[ 'parent_slug' ];
			$page_title		   = $this->submenu_page_options[ 'page_title' ];
			$menu_slug		   = $this->submenu_page_options[ 'menu_slug' ];
			$callback_function = $this->submenu_page_options[ 'callback_function' ];

			add_submenu_page( 'edit.php?post_type=' . $parent_slug, esc_html__( $page_title, $this->textdomain ), esc_html__( $page_title, $this->textdomain ), 'edit_posts', $menu_slug, $callback_function );									
		}
			...
			switch( $column ) {
				...
				case 'icon' :
					...
					if ( has_post_thumbnail() ) {
						...
						echo '<a href="' . $link . '"><img width="60" height="60" src="'. KRAFT_CORE_URL . 'assets/images/no-featured-image.png' .'" alt="' . $post->post_title . '" /></a>';
